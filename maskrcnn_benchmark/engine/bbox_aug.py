...
from maskrcnn_benchmark.modeling.roi_heads.box_head.inference import make_roi_box_post_processor, make_roi_ratio_post_processor
...
def im_detect_bbox_aug(model, images, device):
    ...
    for i, boxlist_ts in enumerate(boxlists_ts):
        ...
        if boxlist_ts[0].has_field('ratios'):
            ratios = torch.cat([boxlist_t.get_field('ratios') for boxlist_t in boxlist_ts])
            boxlist.add_field('ratios', ratios)
        if boxlist_ts[0].has_field('rboxes'):
            rboxes = torch.cat([boxlist_t.get_field('rboxes') for boxlist_t in boxlist_ts])
            boxlist.add_field('rboxes', rboxes)
        boxlists.append(boxlist)

    # Apply NMS and limit the final detections
    results = []
    if cfg.MODEL.RATIO_ON:
        post_processor = make_roi_ratio_post_processor(cfg)
    else:
