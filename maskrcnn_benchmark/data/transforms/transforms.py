...
class RandomRotate90( object ):
    ...
    def __call__( self, image, target=None ):
        ...
        if random.random() < self.prob:
            image = TT.transforms.rotate( image, 90, expand=True )
...
class RandomRotateAug( object ):
    ...
    def __call__( self, image, target=None ):
        ...
        image = TT.functional.rotate( image, 90 * indx, expand=True )
        for _ in range(indx):
            target = target.rotate90()
        return image, target
...
class RandomCrop( object ):
    ...
    def __call__( self, image, target ):
        ...
        image = TT.functional.crop( image, i, j, self.crop_size[0], self.crop_size[1] ) #image[i:i+width,j+height,:]
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        image = TT.functional.normalize(image, mean=self.mean, std=self.std)
        if target is None:
            return image
        return image, target
